...
class InformationRetrieval():
    ...
    def buildIndex(self, docs, docIDs, method):
        """
                ...
                if data[docid][word] != 0:
                    if method=='glove':
                        if word in self.glove_vectors.key_to_index.keys():
                            if doc is not None:
                                doc += (data['IDF'][word] * data[docid][word]) * self.glove_vectors[word]
                            else:
                                doc = (data['IDF'][word] * data[docid][word]) * self.glove_vectors[word]
                    elif method=='w2v':
            ...
            for word in data_full.index:
                if data_full['Query'][word] != 0:
                    if method=='glove':
                        if word in self.glove_vectors.key_to_index.keys():
                            if qvec is not None:
                                qvec += (data_full['IDF'][word] * data_full['Query'][word]) * self.glove_vectors[word]
                            else:
                                qvec = (data_full['IDF'][word] * data_full['Query'][word]) * self.glove_vectors[word]
                    elif method=='w2v':
                        if word in self.word2vec.wv.key_to_index.keys():
